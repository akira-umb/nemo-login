...
def run(config_filepath):
    ...
    for section in sections:
        ...
        elif 'export-to-terra' in section:
            entity_type = section.split("-")[-2] #export-to-terra-file-1 --> file | sample
            section_name_attr = 'export-to-terra-' + entity_type
            
            if 'export-to-terra' not in json_object['cart']:
                json_object['cart']['export-to-terra'] = dict()

            if entity_type not in json_object['cart']['export-to-terra']:
                json_object['cart']['export-to-terra'][entity_type] = dict()
                
            if section_name_attr in section:
                if 'attributes' not in json_object['cart']['export-to-terra'][entity_type]:
                    json_object['cart']['export-to-terra'][entity_type]['attributes'] = list()
                key_vals = format_key_val_pairs( config.items(section) )
                json_object['cart']['export-to-terra'][entity_type]['attributes'].append(key_vals)
