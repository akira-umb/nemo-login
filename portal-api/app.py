...
def get_samples():
    ...
    if request.get_data():
        f1 = request.get_data(as_text=True)
        f2 = json.loads(f1)
        #filters = f2['filters']
        from_str = f2['from']
        order = f2['sort']
...
def get_projects_visualization_data():
    ...
    if graph_type == 'bar_graph':
        ...
        data = ("{{\"data\" : {{\"aggregations\": {0}, \"hits\" : {1}, \"pagination\": {2}}}, \"warnings\": {{}}}}".format(agg_str, hit_str, p_str))
        return make_json_response(data)
...
def send_to_terra():
    ...
    # Get file and sample data. Can be considered raw, unprocessed entities
    (file_data, sample_data) = handler.get_terra_data(request)

    # Process data into complete Terra JSON entities
    json_entities = handler.build_json(file_data, sample_data)
